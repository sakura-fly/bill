...
public class MsgDao extends BaseDaoImpl<Msg> {
    ...
    public double allPrice(Msg msg, String starttime, String endtime){
        ...
        try {
            ...
            NativeQuery q = session.createSQLQuery("select sum( price) from Msg where " + (msg.getUserid() == 0 ? "" : "userid = :userid and ") + "time > :starttime and time < :endtime");
            if (msg.getUserid() != 0 )
                q.setParameter("userid",msg.getUserid());
            q.setParameter("starttime",sdf.parse(starttime));
            q.setParameter("endtime",sdf.parse(endtime));

            r = (double) q.list().get(0);
            tx.commit();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
